...
+ (NSString *)baseUserAgent {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
		NSString *systemName = nil;//[[[UIDevice currentDevice] systemName] stringByReplacingOccurrencesOfString:@" " withString:@"-"];
        if (!systemName) {
            systemName = AWSServiceConfigurationUnknown;
        }
		NSString *systemVersion = nil;//[[UIDevice currentDevice] systemVersion];
        if (!systemVersion) {
